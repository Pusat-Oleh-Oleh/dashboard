...
import { getAllArticles, updateArticle, uploadArticleCover, uploadArticleImages } from '../api/article';
    ...
    const fetchData = async () => {
      try {
        ...
        if (article.cover?.url) {
          setCoverPreview(normalizeUrl(article.cover.url));
        }

        if (article.images?.length > 0) {
          const imagePreviews = article.images.map(img => normalizeUrl(img.url));
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (formData.coverImage instanceof File) {
        await uploadArticleCover(id, formData.coverImage);
      }

      // Upload new article images if provided
      if (formData.images.length > 0 && formData.images.some(img => img instanceof File)) {
        const newImages = formData.images.filter(img => img instanceof File);
        await uploadArticleImages(id, newImages);
      }
          ...
          <div className="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-5xl mx-auto">
            ...
            <div className="mb-8">
              <div className="flex items-center space-x-4 mb-6">
                ...
                <div>
                  <h1 className="text-2xl md:text-3xl text-gray-800 dark:text-gray-100 font-bold">Edit Article</h1>
                  <p className="text-gray-500 dark:text-gray-400">Update your blog article</p>
                </div>
              </div>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6">
                  ...
                  {coverPreview && (
                    <div className="mb-6 relative group">
                      ...
                      <div className="mt-3 text-center">
                        ...
                        <p className="text-xs text-gray-500 dark:text-gray-400">Upload new image to replace</p>
                      </div>
                    </div>
                  )}
                ...
                <div className="mb-6">
                  ...
                  {imagesPreviews.length > 0 && (
                    <div className="mt-6">
                      <h4 className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-3">
                        Current Article Images ({imagesPreviews.length})
                      </h4>
                  ...
                  <p className="mt-2 text-xs text-gray-500 dark:text-gray-400">
                    Upload new images to add to your article
                  </p>
                </div>
